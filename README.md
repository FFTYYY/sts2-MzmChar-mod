...
2. 将解压得到的文件夹 `MzmChar/` 和 `BaseLib/` 复制到 `<游戏目录>/mods/` 中
   - 本 mod 依赖 [BaseLib](https://github.com/Alchyr/BaseLib-StS2)。为了方便使用，本 mod 的安装包中已经自带了一份 BaseLib 的副本。如果你已经装了BaseLib，则可以跳过 BaseLib/ 目录，只复制 `MzmChar/`。
...
- 全新角色「若叶睦」+ 双形态（小睦 / 小墨）机制
- 88 张专属卡牌、专属遗物
- 战斗中可选择切换 BGM 为角色专属曲目（Settings → Mods → 若叶睦角色 → 启用自定义战斗 BGM）
- 中英双语 UI
