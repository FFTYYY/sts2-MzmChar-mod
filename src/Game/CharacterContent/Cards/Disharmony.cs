...
public class Disharmony : MzmCharBaseCard
{
    ...
    private readonly List<DynamicVar> _vars = new()
    {
        ...
        new DynamicVar("DmgHits", 4m),
        new DynamicVar("BlockHits", 2m),
    };
    protected override IEnumerable<DynamicVar> CanonicalVars => _vars;

    public Disharmony() : base(0, CardType.Attack, CardRarity.Ancient, TargetType.AllEnemies)
    {
        SetDefaultReplayCount(1);  // 直接写 field，绕开 set_BaseReplayCount 的 AssertMutable
    }
    ...
    public override List<(string, string)>? Localization => LocManager.Instance.Language switch
    {
        "zhs" => new CardLoc("不和谐音",
            "对全体敌人造成{Damage:diff()}点伤害{DmgHits}次。获得{Block:diff()}点[gold]格挡[/gold]{BlockHits}次。"),
        _ => new CardLoc("Disharmony",
            "Deal {Damage:diff()} damage to ALL enemies {DmgHits} times. Gain {Block:diff()} [gold]Block[/gold] {BlockHits} times."),
    };
}
