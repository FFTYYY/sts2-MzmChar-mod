...
public class TearMaskGold : MzmCharBaseCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext ctx, CardPlay play)
    {
        ...
        {
            // Mu: 获得 N 格挡 × Hits 次 (单独 GainBlock，保留"多次行为"的语义)；不切形态
    ...
    public override List<(string, string)>? Localization => LocManager.Instance.Language switch
    {
        "zhs" => new CardLoc("撕下面具",
            ...
            "{MoSec}{MoOpen}小墨{MoClose}：获得{Energy:energyIcons()}，获得{VigorPower}点[gold]活力[/gold]。[gold]进入小睦[/gold]。{MoSecEnd}"),
        _ => new CardLoc("Tear Off Mask",
            "{MuSec}{MuOpen}Mu{MuClose}: Gain {Block:diff()} [gold]Block[/gold] {Hits} times.{MuSecEnd}\n" +
            "{MoSec}{MoOpen}Mo{MoClose}: Gain {Energy:energyIcons()}; gain {VigorPower} [gold]Vigor[/gold]. [gold]Enter Mu[/gold].{MoSecEnd}"),
    };
}
